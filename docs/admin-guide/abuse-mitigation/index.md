...
Anonymous users are a special case, as they do not have to create an account to interact with your site. This makes it easier for bad actors to create multiple accounts to spam your site.

To mitigate this risk, you can enable a CAPTCHA for anonymous account creations beyond a certain threshold using the [backend configuration](../../config/index.md#anonymous-commenting). This will reduce the risk of spam generated by automated scripts, while still allowing genuine users to interact with comments with zero friction.

Additionally, it's recommended that anonymous comments are sent to the [Manual Moderation Queue](../../core-functionality/manual-moderation/index.md); moderation can be enabled [only for anonymous users](../../config/index.md#manual-moderation) if desired.

Additionally, make use of [Automoderation rules](../../core-functionality/auto-moderation/index.md#anonymous-comments) to manage comments created by anonymous users if required.